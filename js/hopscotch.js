...
(function(global, factory) {
  ...
  var hasJquery = (typeof jQuery === 'undefined' ? 'undefined' : _typeof(
    jQuery)) !== undefinedStr;
  ...
  utils = {
    ...
    getPixelValue: function getPixelValue(val) {
      var valType = typeof val === 'undefined' ? 'undefined' :
        _typeof(val);
      if (valType === 'number') {
        return val;
      }
    ...
    addEvtListener: function addEvtListener(el, evtName, fn) {
      if (el) {
        return el.addEventListener ? el.addEventListener(evtName, fn,
          false) : el.attachEvent('on' + evtName, fn);
      }
    },
    ...
    removeEvtListener: function removeEvtListener(el, evtName, fn) {
      if (el) {
        return el.removeEventListener ? el.removeEventListener(
          evtName, fn, false) : el.detachEvent('on' + evtName, fn);
      }
    },
    ...
    flipPlacement: function flipPlacement(step) {
      if (step.isRtl && !step._isFlipped) {
        ...
        for (i in props) {
          prop = props[i];
          if (step.hasOwnProperty(prop) && rtlMatches.hasOwnProperty(
              step[prop])) {
  ...
  HopscotchBubble.prototype = {
    ...
    setPosition: function setPosition(step) {
      ...
      verticalLeftPosition = step.isRtl ? boundingRect.right -
        bubbleBoundingWidth : boundingRect.left;
      ...
      } else if (step.placement === 'top' || step.placement ===
        'bottom') {
        arrowEl.style.top = '';
        if (arrowOffset === 'center') {
          arrowEl.style[arrowPos] = Math.floor(bubbleBoundingWidth /
            2 - arrowEl.offsetWidth / 2) + 'px';
        } else {
          // Numeric pixel value
          arrowEl.style[arrowPos] = arrowOffset + 'px';
        }
      } else if (step.placement === 'left' || step.placement ===
        'right') {
        arrowEl.style[arrowPos] = '';
        if (arrowOffset === 'center') {
          arrowEl.style.top = Math.floor(bubbleBoundingHeight / 2 -
            arrowEl.offsetHeight / 2) + 'px';
        } else {
          // Numeric pixel value
      ...
      if (step.xOffset === 'center') {
        left = boundingRect.left + targetEl.offsetWidth / 2 -
      ...
      if (step.yOffset === 'center') {
        top = boundingRect.top + targetEl.offsetHeight / 2 -
    ...
    render: function render(step, idx, callback) {
      ...
      if (this.opt.isTourBubble) {
        ...
        if (currTour) {
          ...
          step.isRtl = step.hasOwnProperty('isRtl') ? step.isRtl :
            currTour.hasOwnProperty('isRtl') ? currTour.isRtl : this.opt
            .isRtl;
          unsafe = currTour.unsafe;
          if (Array.isArray(currTour.steps)) {
            totalSteps = currTour.steps.length;
            totalStepsI18n = this._getStepI18nNum(this._getStepNum(
              totalSteps - 1));
            isLast = this._getStepNum(idx) === this._getStepNum(
              totalSteps - 1);
          }
        }
      } else {
        ...
        unsafe = step.unsafe;
        step.isRtl = step.hasOwnProperty('isRtl') ? step.isRtl : this
          .opt.isRtl;
      }
      ...
      opts = {
        ...
        buttons: {
          showPrev: utils.valOrDefault(step.showPrevButton, this.opt
            .showPrevButton) && this._getStepNum(idx) > 0,
          showNext: utils.valOrDefault(step.showNextButton, this.opt
            .showNextButton),
          showCTA: utils.valOrDefault(step.showCTAButton && step.ctaLabel,
            false),
          ctaLabel: step.ctaLabel,
          showClose: utils.valOrDefault(this.opt.showCloseButton,
            true)
        },
        step: {
          num: idx,
      ...
      if (typeof tourSpecificRenderer === 'function') {
        ...
        if (!winHopscotch.templates || typeof winHopscotch.templates[
            tourSpecificRenderer] !== 'function') {
          throw new Error('Bubble rendering failed - template "' +
            tourSpecificRenderer + '" is not a function.');
        }
        ...
        if (!winHopscotch.templates || typeof winHopscotch.templates[
          ...
          throw new Error('Bubble rendering failed - template "' +
            templateToUse + '" is not a function.');
        }
    ...
    _handleBubbleClick: function _handleBubbleClick(evt) {
      ...
      if (action === 'cta') {
        if (!this.opt.isTourBubble) {
          ...
          winHopscotch.getCalloutManager().removeCallout(this.currStep
            .id);
        }
    ...
    init: function init(initOpt) {
      ...
      initOpt = (typeof initOpt === 'undefined' ? 'undefined' :
        _typeof(initOpt)) === undefinedStr ? {} : initOpt;
  ...
  HopscotchCalloutManager = function HopscotchCalloutManager() {
    ...
    this.refreshCalloutPositions = function() {
      ...
      for (calloutId in callouts) {
        if (callouts.hasOwnProperty(calloutId) && calloutOpts.hasOwnProperty(
            calloutId)) {
  ...
  Hopscotch = function Hopscotch(initOptions) {
    var self = this,
      ...
      getCurrStep = function getCurrStep() {
        ...
        if (!currTour || currStepNum < 0 || currStepNum >= currTour.steps
          .length) {
          step = null;
        } else {
      ...
      adjustWindowScroll = function adjustWindowScroll(cb) {
        var bubble = getBubble(),
          ...
          targetBottom = bubbleBottom > targetElBottom ? bubbleBottom :
          targetElBottom,
        ...
        if (targetTop >= windowTop && (targetTop <= windowTop +
            getOption('scrollTopMargin') || targetBottom <=
            windowBottom)) {
          if (cb) {
            cb();
        ...
        // Smooth scroll to scroll target
          ...
          if ((typeof YAHOO === 'undefined' ? 'undefined' : _typeof(
              YAHOO)) !== undefinedStr && _typeof(YAHOO.env) !==
            undefinedStr && _typeof(YAHOO.env.ua) !== undefinedStr &&
            _typeof(YAHOO.util) !== undefinedStr && _typeof(YAHOO.util.Scroll) !==
            undefinedStr) {
            scrollEl = YAHOO.env.ua.webkit ? document.body : document.documentElement;
            yuiEase = YAHOO.util.Easing ? YAHOO.util.Easing.easeOut :
              undefined;
          ...
          // Use my crummy setInterval scroll solution if we're using plain, vanilla Javascript.
            ...
            scrollIncr = Math.abs(windowTop - scrollToVal) / (getOption(
              'scrollDuration') / 10);
      ...
      goToStepWithTarget = function goToStepWithTarget(direction, cb) {
        ...
        if (currStepNum + direction >= 0 && currStepNum + direction <
          currTour.steps.length) {
      ...
      changeStep = function changeStep(doCallbacks, direction) {
        ...
        if (step.nextOnTargetClick) {
          ...
          utils.removeEvtListener(utils.getStepTarget(step), 'click',
            targetClickNextFn);
        }
        ...
        if (direction > 0) {
          ...
          wasMultiPage = currStepNum > 0 && currTour.steps[currStepNum -
            1].multipage;
        }
        ...
        changeStepCb = function changeStepCb(stepNum) {
          ...
          if (doCallbacks) {
            if (direction > 0) {
              doShowFollowingStep = utils.invokeEventCallbacks('next',
                origStep.onNext);
            } else {
              doShowFollowingStep = utils.invokeEventCallbacks('prev',
                origStep.onPrev);
            }
          }
        ...
        } else if (currStepNum + direction >= 0 && currStepNum +
          direction < currTour.steps.length) {
      ...
      loadTour = function loadTour(tour) {
        ...
        for (prop in tour) {
          if (tour.hasOwnProperty(prop) && prop !== 'id' && prop !==
            'steps') {
       ...
       * target on the page?)
       */
      findStartingStep = function findStartingStep(startStepNum,
        savedSkippedSteps, cb) {
        var step, target;
      ...
      showStepHelper = function showStepHelper(stepNum) {
        ...
        if (currStepNum !== stepNum && getCurrStep().nextOnTargetClick) {
          // Detach the listener when tour is moving to a different step
          utils.removeEvtListener(utils.getStepTarget(getCurrStep()),
            'click', targetClickNextFn);
        }
        ...
        bubble.render(step, stepNum, function(adjustScroll) {
          ...
          if (step.nextOnTargetClick) {
            utils.addEvtListener(targetEl, 'click',
              targetClickNextFn);
          }
        });
    ...
    this.getCalloutManager = function() {
      if ((typeof calloutMgr === 'undefined' ? 'undefined' : _typeof(
          calloutMgr)) === undefinedStr) {
    ...
    this.startTour = function(tour, stepNum) {
      ...
      if ((typeof stepNum === 'undefined' ? 'undefined' : _typeof(
          stepNum)) !== undefinedStr) {
      ...
      if (typeof currStepNum === "undefined" && currTour.id ===
        cookieTourId && (typeof cookieTourStep === 'undefined' ?
          'undefined' : _typeof(cookieTourStep)) !== undefinedStr) {
      ...
      findStartingStep(currStepNum, skippedSteps, function(stepNum) {
        var target = stepNum !== -1 && utils.getStepTarget(
          currTour.steps[stepNum]);
    ...
    this.endTour = function(clearState, doCallbacks) {
      ...
      if (currTour) {
        ...
        if (currentStep && currentStep.nextOnTargetClick) {
          utils.removeEvtListener(utils.getStepTarget(currentStep),
            'click', targetClickNextFn);
        }
      }
    ...
    _configure = function _configure(options, isTourOptions) {
      var bubble,
        events = ['next', 'prev', 'start', 'end', 'show', 'error',
          'close'
        ],
        eventPropName,
        callbackProp,
      ...
      for (i = 0, len = events.length; i < len; ++i) {
        ...
        // to a function.
        eventPropName = 'on' + events[i].charAt(0).toUpperCase() +
    ...
    this.setRenderer = function(render) {
      var typeOfRender = typeof render === 'undefined' ? 'undefined' :
        _typeof(render);
